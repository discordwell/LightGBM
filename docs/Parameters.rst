...
-  ``device_type`` :raw-html:`<a id="device_type" title="Permalink to this parameter" href="#device_type">&#x1F517;&#xFE0E;</a>`, default = ``cpu``, type = enum, options: ``cpu``, ``gpu``, ``cuda``, aliases: ``device``
   ...
   -  ``metal`` uses Apple Metal for GPU-accelerated training on macOS with Apple Silicon

   -  **Note**: ``metal`` is performance-first and currently supports only ``tree_learner=serial``, ``boosting=gbdt``, dense numerical features, and ``max_bin <= 256``
   ...
   -  **Note**: ``metal`` always uses 32-bit float. ``gpu_use_dp`` is not supported with Metal

   -  **Note**: refer to `Installation Guide <./Installation-Guide.rst>`__ to build LightGBM with GPU, CUDA, ROCm, or Metal support
